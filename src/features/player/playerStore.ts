...
export const usePlayerStore = create<PlayerState & PlayerStoreActions>(
  (set, get) => {
    ...
    const initialFavorites = safeGetItem("Resona_favorites", []) as string[];
    const initialHistory = safeGetItem("Resona_history", []) as Track[];
    const initialPlaylists = safeGetItem("Resona_playlists", []) as Playlist[];
    const initialVolume = safeGetItem("Resona_volume", 0.8) as number;
    ...
    return {
      ...
      setVolume: (volume) => {
        set({ volume });
        safeSetItem("Resona_volume", volume);
      },
      ...
      toggleFavorite: (trackId) => {
        ...
        safeSetItem("Resona_favorites", updated);
      },
      ...
      removeFromQueue: (trackId) => {
        ...
        if (state.currentTrack?.id === trackId) {
          ...
          newIndex = updatedQueue.findIndex(
      ...
      addToHistory: (track) => {
        ...
        set({ history: updated });
        safeSetItem("Resona_history", updated);
      },
      ...
      createPlaylist: (name, description) => {
        ...
        safeSetItem("Resona_playlists", updated);
      },
      ...
      setImmersivePlayerOpen: (immersivePlayerOpen) =>
        set({ immersivePlayerOpen }),
    };
  },
);
