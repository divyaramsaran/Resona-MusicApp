...
const REGIONAL_STATIONS = [
  {
    ...
    tags: 'telugu',
    cover: 'https://images.unsplash.com/photo-1513829096999-4978602297a7?w=400&q=80',
  },
  {
    id: 'hindi',
    ...
    tags: 'hindi',
    cover: 'https://images.unsplash.com/photo-1585647347483-22b66260dfff?w=400&q=80',
  },
  {
    id: 'tamil',
...
export default function HomeTab() {
  ...
  const { playTrack, setActiveTab, setSelectedGenre, setSearchQuery } = usePlayerStore();

  useEffect(() => {
    async function loadTrending() {
      try {
        const topTracks = await fetchJamendoTracks({ limit: 10, order: 'popularity_total' });
  ...
  const handleMoodPlay = async (tag: string) => {
    ...
    try {
      ...
      const isRegional = ['telugu', 'hindi', 'tamil', 'malayalam'].includes(tag.toLowerCase());
      if (isRegional) {
  ...
  return (
    <div id="home-tab-view" className="space-y-8 w-full max-w-4xl mx-auto pb-28">
      ...
      <div>
        ...
        <p className="text-xs sm:text-sm text-zinc-400 mt-1 font-sans">
          Welcome to Harmony. Tap any card below to dive straight into licensed, ad-free ambient sound streams.
        </p>
      </div>
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-lg font-bold text-white tracking-tight">Curated Mood Stations</h3>
        </div>
          ...
          {MOOD_PLAYLISTS.map((mood) => (
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <h4 className="text-lg font-bold text-white mt-3 tracking-tight">{mood.title}</h4>
                <p className="text-xs text-zinc-300 mt-1 leading-relaxed font-sans">{mood.subtitle}</p>
              </div>
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-lg font-bold text-white tracking-tight">Indian Cinema Stations</h3>
        </div>
          ...
          {REGIONAL_STATIONS.map((station) => (
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <h4 className="text-lg font-bold text-white mt-3 tracking-tight">{station.title}</h4>
                <p className="text-xs text-zinc-300 mt-1 leading-relaxed font-sans">{station.subtitle}</p>
              </div>
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-bold text-white tracking-tight">Trending on Jamendo</h3>
          </div>
          <span className="text-xs font-mono text-slate-500">POPULAR LICENSES</span>
        </div>

        {loading ? (
          <div className="flex flex-col items-center justify-center py-16 gap-2 text-slate-500">
            <Loader2 className="w-7 h-7 animate-spin text-indigo-400" />
            <span className="text-xs font-mono">Curating popular legal tracks...</span>
          </div>
        ) : trending.length > 0 ? (
